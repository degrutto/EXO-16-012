...
void MakePlots(const Events * ev, TString var,
  ...
  if ( newnbinsx == -1) newnbinsx = nbinsx;
    ...
    */
	...
	if (plotBoostedZH)  leg1->AddEntry(hboostedZH, "Z->ZH(MZ'=3000), xsex = 1pb", "l");
    ...
    if (printStat) {
      ...
      double nS_err = -1; 
      if (plotBoostedZH) {
	 nS = hboostedZH->IntegralAndError(0,hboostedZH->GetNbinsX(), nS_err);
      } else {
	...
	//nggZHS = hggZH->IntegralAndError(0,hggZH->GetNbinsX(), nggZHS_err);
      ...
      double nboostedZH = hboostedZH->IntegralAndError(0,hboostedZH->GetNbinsX(), nboostedZH_err);
	...
	if (plotData) {
	  double nData = hdata_obs->IntegralAndError(0, hVV->GetNbinsX(), nVV_err);
    ...
    if (plotBoostedZH)  leg1->AddEntry(hboostedZH, "MZ'=3000, xsec=1pb", "l");
    leg1->AddEntry(hTT, "t#bar{t}", "f");
    leg1->AddEntry(hST, "single top", "f");
    leg1->AddEntry(hVV, "VV", "f");
    ...
    //   latex->DrawLatex(0.19, 0.79, "Z(#nu#bar{#nu})H(b#bar{b})");
      if (plotBoostedZH)  latex->DrawLatex(0.19, 0.79, "Z'->Z(#nu#bar{#nu})H(b#bar{b})");
...
void MakeDatacard(TString channel, TString dcname, TString wsname,
    ...
    std::clog << "MakeDatacard(): got ggZH " << std::endl;
      ...
      double nboostedZH_err = -1;
      double nboostedZH =  ((TH1 *) wsfile->Get("boostedZH"))->IntegralAndError(0,  999 , nboostedZH_err);
    ...
    dc << "lumi_13TeV                           lnN    1.026  1.026  1.026 1.026 -     -     -     -     -     1.026 1.026 1.026 " << std::endl;
    ...
    dc << "CMS_vhbb_WjHF_SF_" << channel <<"    lnN    -      -     -     -     -     1.50  -     -     -     -     -      -     " << std::endl;
    dc << "CMS_vhbb_ZjLF_SF_" << channel <<"    lnN    -      -     -     -     -     -     1.08  -     -     -     -      -     " << std::endl;
    dc << "CMS_vhbb_ZjHF_SF_" << channel <<"    lnN    -      -     -     -     -     -     -     1.50  -     -     -      -     " << std::endl;
    dc << "CMS_vhbb_TT_SF_"   << channel <<"    lnN    -      -     -     -     -     -     -     -     1.07  -     -      -     " << std::endl;

    dc << "CMS_vhbb_boostedZH_stat                   lnN    "<< 1.+nboostedZH_err/nboostedZH<<"  -   -     -     -     -     -     -     -     -     -     -     " << std::endl;
    ...
    dc << "CMS_vhbb_trigger_MET                 lnN    1.03   1.03    1.03  1.03  -     -     -     -     -     1.03  1.03  1.03 " << std::endl;
    dc << "#################################### #####  boostedZH    ggZH    ZH    WH    WjLF  WjHF  ZjLF  ZjHF  TT    ST    VV    QCD  " << std::endl;
    dc.close();
...
TString indir = "/afs/cern.ch/work/d/degrutto/public/MiniAOD/ZnnHbb_v14/step3/"; //Step4_ZnnH125.root
    ...
    bool loadboostedZH   = processes.Contains("boostedZH")  ;
    ...
    if (loadboostedZH) {
        ...
        //ZH = (TTree *) ZH_.CopyTree(cutmc_all);
        //lumi_ZH = lumis[Form("ZllH%i", massH)];
        boostedZH_.Add(indir + prefix + "boostedZH" + suffix);
