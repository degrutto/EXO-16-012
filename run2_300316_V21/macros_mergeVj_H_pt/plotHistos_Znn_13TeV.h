...
void MakePlots(const Events * ev, TString var,
               ...
               TString title, int nbinsx, double xlow, double xup,
               TString plotname="plot", TString plotdir="",
               TString options="printStat:plotSig:plotData:plotLog:!plotNorm:!plotMonoH", bool doRebin = true , int newnbinsx  = -1) {
    ...
    */
    ...
    if (doRebin) {
      ...
      std::cout << "rebinning ################################" << std::endl;  
        ...
        hmonoH        = rebinner->rebin(hmonoH         , newnbinsx  , "monoH"        );
	...
	//nggZHS = hggZH->IntegralAndError(0,hggZH->GetNbinsX(), nggZHS_err);
      }
      double nVH_err = -1;
      double nVH = hVH->IntegralAndError(0,hVH->GetNbinsX(), nVH_err);

      double nggZH_err = -1;
      double nggZH = hggZH->IntegralAndError(0,hggZH->GetNbinsX(), nggZH_err);
      ...
      double nQCD_err =-1;
      double nQCD = hQCD->IntegralAndError(0, hQCD->GetNbinsX(), nQCD_err);


      double nVV_err =-1;
      double nVV = hVV->IntegralAndError(0, hVV->GetNbinsX(), nVV_err);
...
void MakeDatacard(TString channel, TString dcname, TString wsname,
    ...
    TFile * wsfile = TFile::Open(wsname);
      ...
      double nZH_err = -1;
      double nZH =  ((TH1 *) wsfile->Get("ZH"))->IntegralAndError(0,  999 , nZH_err);

      double nWH_err = -1;
      double nWH =  ((TH1 *) wsfile->Get("WH"))->IntegralAndError(0,  999 , nWH_err);

      double nggZH_err = -1;
      double nggZH =  ((TH1 *) wsfile->Get("ggZH"))->IntegralAndError(0,  999 , nggZH_err);


      double nZjHF_err =-1; 
      double nZjHF =  ((TH1 *) wsfile->Get("ZjHF"))->IntegralAndError(0,  999 , nZjHF_err);

      double nZjLF_err =-1; 
      double nZjLF =  ((TH1 *) wsfile->Get("ZjLF"))->IntegralAndError(0,  999 , nZjLF_err);


      double nWjHF_err =-1; 
      double nWjHF =  ((TH1 *) wsfile->Get("WjHF"))->IntegralAndError(0,  999 , nWjHF_err);

      double nWjLF_err =-1; 
      double nWjLF =  ((TH1 *) wsfile->Get("WjLF"))->IntegralAndError(0,  999 , nWjLF_err);

      double nTT_err =-1; 
      double nTT =  ((TH1 *) wsfile->Get("TT"))->IntegralAndError(0,  999 , nTT_err);



      double nST_err =-1; 
      double nST =  ((TH1 *) wsfile->Get("ST"))->IntegralAndError(0,  999 , nST_err);

      double nVV_err =-1; 
      double nVV =  ((TH1 *) wsfile->Get("VV"))->IntegralAndError(0,  999 , nVV_err);


      double nQCD_err =-1; 
      double nQCD=  ((TH1 *) wsfile->Get("QCD"))->IntegralAndError(0,  999 , nQCD_err);
    ...
    if (unblind) {
        ...
        dc << "observation " << ((TH1 *) wsfile->Get("mc_exp"))->Integral() + ((TH1 *) wsfile->Get("ZH"))->Integral() + ((TH1 *) wsfile->Get("ggZH"))->Integral()  + ((TH1 *) wsfile->Get("WH"))->Integral() << std::endl;
    } else {
    ...
    dc << "process     ggZH      ZH         WH         WjLF       WjHF       ZjLF       ZjHF       TT         ST         VV       QCD  " << std::endl;
    dc << "process     -2        -1         0          1          2          3          4          5          6          7        8    " << std::endl;
    dc << "rate        " << setw(10) << right << ((TH1 *) wsfile->Get("ggZH"))->Integral() << " "
    ...
    dc << "#################################### #####  ggZH   ZH    WH    WjLF  WjHF  ZjLF  ZjHF  TT    ST    VV    QCD " << std::endl;
    dc << "lumi_13TeV                           lnN    1.026  1.026 1.026 -     -     -     -     -     1.026 1.026 1.026 " << std::endl;
    dc << "pdf_qqbar                            lnN    -      1.01  1.01  -     -     -     -     -     -     1.01  -     " << std::endl;
    dc << "pdf_gg                               lnN    1.30   -     -     -     -     -     -     1.01  -     -     1.01  "  << std::endl;
    ...
    dc << "CMS_vhbb_eff_b                       lnN    1.07   1.07  1.07  1.07  1.07  1.07  1.07  1.07  1.07  1.07  1.07  " << std::endl;
    dc << "CMS_vhbb_fake_b_13TeV                lnN    1.03   1.03  1.03  1.03  1.03  1.03  1.03  1.03  1.03  1.03  1.03  " << std::endl;
    dc << "CMS_vhbb_res_j                       lnN    1.05   1.05  1.05  1.05  1.05  1.05  1.05  1.05  1.05  1.05  1.05  " << std::endl;
    dc << "CMS_vhbb_scale_j                     lnN    1.05   1.05  1.05  1.05  1.05  1.05  1.05  1.05  1.05  1.05  1.05  " << std::endl;
