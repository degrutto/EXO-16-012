...
void findLastN(UInt_t& lastN, const UInt_t end, const TH1F* bkg, const double errorf) {
    ...
    while ((lastbinerrorf > errorf) && (lastN <= (UInt_t) bkg->GetNbinsX())) {
        bkg->Info("Rebin", "lastN=%2i int. binc=%f int. bine=%f frac=%f", lastN, lastbincontent, sqrt(lastbinerror2), lastbinerrorf);
    ...
    TH1F * rebin(const TH1F * h, long long nb, const char * newname ="")
    {
        ...
        if (!recreate) {
            ...
            for (int ipart=0; ipart < npartitions; ipart++) {
                ...
                //findLastN2(lastN2, end - lastN, sig->Integral(end - lastN, end-1)/sig->Integral(begin, end-1)*1.30, sig->Integral(begin, end-1 - lastN), sig, errorflast);
                
                UInt_t lastN3 = 1;  // starts with 1 bin
                findLastN2(lastN3, end - lastN - lastN2, (1.0 - bkg->Integral(end - lastN - lastN2, end-1 - lastN)/bkg->Integral(begin, end-1 - lastN))*0.99, bkg->Integral(begin, end-1 - lastN - lastN2),  bkg, errorflast );
                //findLastN2(lastN3, end - lastN - lastN2, sig->Integral(end - lastN - lastN2, end-1 - lastN)/sig->Integral(begin, end-1 - lastN)*1.50, sig->Integral(begin, end-1 - lastN - lastN2), sig, errorflast);
		...
		//                int nmiddlebins = oldnbins - firstN - lastN - (lastN -1) - (lastN -2);
                int nmiddlebins = 0.5 * ( lastN - firstN) ;

                int ngroup = newnbins-4;
                int nbg = nmiddlebins / ngroup;
                hdummy->Info("Rebin", "firstN=%2i lastN=%2i lastN2=%2i lastN3=%2i  nmiddlebins=%i before adjusting nmiddlebins", firstN, lastN, lastN2, lastN3, nmiddlebins);
		...
		/*
			...
			//                        nmiddlebins = oldnbins - firstN - lastN - (lastN -1 ) -  (lastN -2);
                        nbg = nmiddlebins / ngroup;
                    }
		     }
		*/ 
		...
		//                lowedges.at(iedge) = in_xaxis->GetBinLowEdge(end - lastN - lastN2);
                lowedges.at(iedge) = in_xaxis->GetBinLowEdge(end - lastN - (lastN -1 ));
                iedge++;
