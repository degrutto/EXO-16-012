...
void MakePlots(const Events * ev, TString var,
               ...
               TString title, int nbinsx, double xlow, double xup,
               TString plotname="plot", TString plotdir="",
               TString options="printStat:plotSig:plotData:plotLog:!plotNorm:!plotBoostedZH", bool doRebin = false , int newnbinsx  = -1 , bool drawOverflow = false) {
  ...
  std::clog << "... DONE: project ZH." << std::endl;
	...
	//        ev->boostedZH->Project("boostedZH", var, cutmc * Form("%5f * 1.", ev->lumi_boostedZH));
        std::clog << "... before project boostedZH." << std::endl;
	 		//TCut effLumiboostedVH = "(3000. * 1.  * ( 0.577 * 0.1 )/ 50000.)";    //mZ'=600, assume 1 pb and scale to 3 fb-1
...
void MakeDatacard(TString channel, TString dcname, TString wsname,
    ...
    if (unblind) {
      ...
      dc << "observation " << ((TH1 *) wsfile->Get("mc_exp"))->Integral() + ((TH1 *) wsfile->Get("ZH"))->Integral() + ((TH1 *) wsfile->Get("ggZH"))->Integral()  + ((TH1 *) wsfile->Get("WH"))->Integral() + ((TH1 *) wsfile->Get("monoH"))->Integral()  << std::endl;
    } else {
    ...
    dc << "process     monoH  ggZH      ZH         WH         Wj          Zj        TT         ST         VV       QCD  " << std::endl;
    dc << "process     -1          9        10         11          1          2          3          4          5          6             " << std::endl;
    ...
    dc << "#################################### #####  monoH ggZH   ZH    WH    Wj   Zj  TT    ST    VV    QCD " << std::endl;
    dc << "lumi_13TeV                           lnN    1.04  1.04  1.04 1.04 -       -     -     1.04 1.04 1.04 " << std::endl;
    dc << "pdf_qqbar                            lnN    1.01    -      1.01  1.01  -        -     -     -     1.01  -     " << std::endl;
    dc << "pdf_gg                               lnN    -   1.50   -     -     -     -         -   1.05     -     1.50  "  << std::endl;
    ...
    dc << "CMS_monoHbb_eff_b                       lnN    1.07   1.07   1.07  1.07   -        -     -    1.07  1.07  1.07  " << std::endl;
    dc << "CMS_monoHbb_fake_b_13TeV                lnN    -   -   -  -   -         -     -    1.03     1.03    -   " << std::endl;
    dc << "CMS_monoHbb_res_j                       lnN    1.05   1.05   1.05  1.05   -        -     -    1.05  1.05  1.05  " << std::endl;
    dc << "CMS_monoHbb_scale_j                     lnN    1.05   1.05   1.05  1.05   -        -    -     1.05  1.05  1.05  " << std::endl;
    ...
    dc << "CMS_monoHbb_trigger_MET                 lnN    1.03   1.03    1.03  1.03  -        -     -     1.03  1.03  1.03 " << std::endl;
    dc << "#################################### #####  monoH    ggZH    ZH    WH    Wj  Zj  TT    ST    VV    QCD  " << std::endl;
    dc.close();
...
TString indir = "/afs/cern.ch/work/d/degrutto/public/MiniAOD/V21_FromSean/step3/"; //Step4_ZnnH125.root
	...
	//       boostedZH_.Add("/afs/cern.ch/user/d/degrutto/eos/cms/store/group/cmst3/user/degrutto/VHBBHeppyV21_add/ZprimeToA0hToA0chichihbb_2HDM_MZp-800_MA0-300_13TeV-madgraph/VHBB_HEPPY_F21_add_ZprimeToA0hToA0chichihbb_2HDM_MZp-800_MA0-300_13TeV-madgraph__fall15MAv2-pu25ns15v1_76r2as_v12-v1/160407_124903/0000/tree_4.root");
