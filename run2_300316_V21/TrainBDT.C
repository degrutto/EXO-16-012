...
const TString indir     = "/afs/cern.ch/work/d/degrutto/public/MiniAOD/ZnnHbb_Spring15_PU20bx25/skimV12_v2/step3/skim_ZnnH_classification/";
...
const int pars_ZnunuHighPt[4] = {300, 2, 3, 10};  // NTrees, MinNodeSize*100  , MaxDepth, AdaBoostBeta*100 <-- NTrees from 600 to 300, use MinNodeSize instead of MinNodeSize, 
...
const TMVA::VariableInfo g_variables[] = {
  ...
  // FIXME!!! not use regression for now
    TMVA::VariableInfo("HCSV_mass", 
                       "H mass", "GeV", varcounter++, 'F'),
    TMVA::VariableInfo("HCSV_pt", 
                       "H p_{T}", "GeV", varcounter++, 'F'),
    TMVA::VariableInfo("max(Jet_pt[hJCidx[0]],Jet_pt[hJCidx[1]])", 
                       "H j1 p_{T}", "GeV", varcounter++, 'F'),
    TMVA::VariableInfo("min(Jet_pt[hJCidx[0]],Jet_pt[hJCidx[1]])", 
...
void TrainChannelBDT(const TString& channel, int massH, TString whatbkg, TString whatsig, TString whatvar, const TString& outdir, int level, int NTrees, double MinNodeSize100 , int MaxDepth, int AdaBoostBeta100, int i_parlevel2, const TString& method)
{
    ...
    TFile * WH = TFile::Open(indir + prefix + Form("ggZH%i", massH) + suffix);
    TFile * Wj = TFile::Open(indir + prefix + "WJets" + suffix);
    TFile * Zj = TFile::Open(indir + prefix + "ZJets" + suffix);
    TFile * TT = TFile::Open(indir + prefix + "TTPowheg" + suffix);
 ...
 //    TFile * ZZ = TFile::Open(indir + prefix + "ZZ" + suffix); <-- missing
    ...
    if (bkgAll) {
      ...
      std::cout << "before adding test tree for VV" << std::endl;
  ...
  factory->SetSignalWeightExpression("efflumi * weightTrig * sign(genWeight)");
    // factory->SetBackgroundWeightExpression("weightsMC[0]*(selectFlags[0][0])");
  factory->SetBackgroundWeightExpression("efflumi * weightTrig * sign(genWeight)");
